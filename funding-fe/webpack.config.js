  ...
  resolve: {
    alias: {
      'frontend-config': (mode === 'production') ?
        ...
        ((mode === 'developmentauth') ? path.resolve(__dirname, './modules/frontend-config/devauth') : path.resolve(__dirname, './modules/frontend-config/dev'))
    }
  },
  output: {
...
function getConfig() {
  switch (process.env.npm_lifecycle_event) {
    ...
    case 'start:auth':
      console.log('development dev server with auth');
      return _.mergeWith({}, common('developmentauth'), dev, {
        devServer
      }, customizer);
    case 'start':
    case 'start:dev':
